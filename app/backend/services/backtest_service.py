...
def get_company_news(ticker, end_date, start_date=None, limit=1000, api_key=None):
    """Mock function for getting company news."""
    return []
...
def get_financial_metrics(ticker, end_date, limit=10, api_key=None):
    """Mock function for getting financial metrics."""
    return []
...
class BacktestService:
    ...
    def execute_trade(self, ticker: str, action: str, quantity: float, current_price: float) -> float:
        """
        ...
        if action == "sell":
            ...
            max_quantity = int(self.portfolio["cash"] / (current_price * margin_ratio)) if margin_ratio > 0 else 0
    ...
    async def arun_backtest(self, progress_callback: Callable | None = None) -> dict[str, Any]:
        ...
        for i, current_date in enumerate(dates):
            ...
            portfolio_for_graph = portfolio_service.create_portfolio(
            ...
            try:
                ...
                result = await graph_service.arun_graph(
                    graph=self.graph,
                ...
                if result and result.get("messages"):
                    decisions = graph_service.parse_hedge_fund_response(result["messages"][-1].content)
                ...
                logger.info(f"Error running graph for {current_date_str}: {e}")
                decisions = {}
                analyst_signals = {}
