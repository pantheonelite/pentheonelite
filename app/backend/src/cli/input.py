...
def select_analysts(flags: dict | None = None) -> list[str]:
    ...
    choices = questionary.checkbox(
        ...
        choices=[questionary.Choice(display, value=value) for display, value in crypto_analyst_order],
        instruction=(
    ...
    analyst_names = ", ".join(Fore.GREEN + c.title().replace("_", " ") + Style.RESET_ALL for c in choices)
    logger.info("\nSelected analysts: %s\n", analyst_names, analysts=choices)
    return choices


def select_model(*, _use_ollama: bool, model_flag: str | None = None) -> tuple[str, str]:
