...
def upgrade():
    ...
    if "trading_mode" not in existing_columns:
        op.add_column(
            "hedge_fund_flow_runs", sa.Column("trading_mode", sa.String(50), nullable=False, server_default="one-time")
        )
    ...
    # Create indexes only if they don't exist (handle case where table exists but indexes might not)
    existing_indexes = [idx["name"] for idx in inspector.get_indexes("hedge_fund_flow_run_cycles")]

    if "ix_hedge_fund_flow_run_cycles_flow_run_id" not in existing_indexes:
        op.create_index("ix_hedge_fund_flow_run_cycles_flow_run_id", "hedge_fund_flow_run_cycles", ["flow_run_id"])
    if "ix_hedge_fund_flow_run_cycles_cycle_number" not in existing_indexes:
        op.create_index("ix_hedge_fund_flow_run_cycles_cycle_number", "hedge_fund_flow_run_cycles", ["cycle_number"])
    if "ix_hedge_fund_flow_run_cycles_status" not in existing_indexes:
        op.create_index("ix_hedge_fund_flow_run_cycles_status", "hedge_fund_flow_run_cycles", ["status"])
    if "ix_hedge_fund_flow_run_cycles_started_at" not in existing_indexes:
...
def downgrade():
    ...
    if "hedge_fund_flow_run_cycles" in existing_tables:
        ...
        try:
            ...
            logger.warning("Index drop skipped", error=str(e))
