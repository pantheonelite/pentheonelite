...
export function ApiKeysSettings() {
  ...
  const renderApiKeySection = (title: string, description: string, keys: ApiKey[], icon: React.ReactNode) => (
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2 font-mythic text-pantheon-text-primary">
          {icon}
          {title}
        </CardTitle>
        <p className="text-sm text-pantheon-text-secondary">{description}</p>
      </CardHeader>
      <CardContent className="space-y-4">
        {keys.map((apiKey) => (
          <div key={apiKey.key} className="space-y-2">
             <button
               className="text-sm font-semibold text-pantheon-primary-500 hover:text-pantheon-primary-600 underline underline-offset-4 text-left transition-colors"
              ...
              <div className="absolute right-1 top-1/2 -translate-y-1/2 flex items-center gap-1">
                {apiKeys[apiKey.key] && (
                  <Button
                    variant="ghost"
                    size="icon"
                    className="h-7 w-7 text-pantheon-accent-red hover:bg-pantheon-cosmic-bg transition-colors"
  ...
  if (loading) {
    return (
      <div className="space-y-6">
        <div>
          <h2 className="font-mythic text-lg font-semibold text-pantheon-text-primary mb-2">API Keys</h2>
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Error Message */}
      {error && (
        <Card className="border-pantheon-accent-red/40 bg-pantheon-accent-red/5">
          <CardContent className="p-4">
            <div className="flex items-start gap-3">
              <Key className="h-5 w-5 text-pantheon-accent-red mt-0.5 flex-shrink-0" />
              <div className="space-y-1">
                <h4 className="text-sm font-semibold uppercase tracking-wider text-pantheon-accent-red">Error</h4>
                <p className="text-sm text-pantheon-text-secondary">{error}</p>
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={() => {
                  ...
                  className="text-xs mt-2 p-0 h-auto text-pantheon-accent-red hover:text-pantheon-accent-red/80 transition-colors"
                >
                  Try again
                </Button>
              </div>
      ...
      {/* Security Note */}
      <Card className="border-pantheon-accent-orange/40 bg-pantheon-accent-orange/5">
        <CardContent className="p-4">
          <div className="flex items-start gap-3">
            <Key className="h-5 w-5 text-pantheon-accent-orange mt-0.5 flex-shrink-0" />
            <div className="space-y-1">
              <h4 className="text-sm font-semibold uppercase tracking-wider text-pantheon-accent-orange">Security Note</h4>
              <p className="text-sm text-pantheon-text-secondary">
