...
export default function FlowItem({ flow, onLoadFlow, onDeleteFlow, onRefresh, isActive = false }: FlowItemProps) {
  ...
  return (
    <>
      <div
        className={cn(
          "group flex items-center justify-between px-4 py-3 transition-all cursor-pointer border-2 shadow-[0_0_5px_hsl(var(--pantheon-primary-500)/0.1)]",
          isActive
            ? "border-pantheon-primary-500 bg-pantheon-cosmic-bg"
            : "border-pantheon-border bg-pantheon-cosmic-surface hover:bg-pantheon-cosmic-bg"
        )}
      ...
      >
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2 mb-1">
            <div className="flex items-center gap-1 min-w-0">
              {flow.is_template ? (
                <Layout size={14} className="flex-shrink-0 text-pantheon-primary-500" />
              ) : (
            ...
            {hasActiveConnection && (
              <div className="flex items-center gap-1 flex-shrink-0">
                <Zap className="h-3 w-3 text-pantheon-accent-orange animate-pulse" />
                <span className="text-[0.6rem] text-pantheon-accent-orange font-semibold uppercase tracking-[0.2em]">Running</span>
              </div>
            )}
          </div>

          <div className="flex items-center gap-2 text-[0.6rem] text-pantheon-text-secondary">
            <Calendar size={10} />
          ...
          {filteredTags.length > 0 && (
            <div className="flex flex-wrap gap-1 mt-1">
              {filteredTags.slice(0, 2).map(tag => (
                <Badge key={tag} variant="secondary" className="text-[0.6rem] px-2 py-0 bg-pantheon-primary-500/20 text-pantheon-primary-500 border-pantheon-primary-500/30">
                  {tag}
                </Badge>
              ))}
              {filteredTags.length > 2 && (
                <Badge variant="secondary" className="text-[0.6rem] px-2 py-0 bg-pantheon-primary-500/20 text-pantheon-primary-500 border-pantheon-primary-500/30">
                  +{filteredTags.length - 2}
                </Badge>
              )}
            </div>
        ...
        <div className="flex items-center">
          <Button
            ...
            className="h-7 w-7 text-pantheon-text-secondary hover:text-pantheon-primary-500 opacity-0 group-hover:opacity-100 transition-all rounded-none"
            title="More options"
          >
