...
export function Flow({ className = '' }: FlowProps) {
  ...
  const gridColor = resolvedTheme === 'light' ? 'var(--tab-border)' : 'var(--tab-close-hover)';

  return (
    <div className={cn("w-full h-full border-2 border-border bg-[hsl(var(--surface-elevated))]", className)}>
      <TooltipProvider>
        <ReactFlow
          nodes={nodes}
          nodeTypes={nodeTypes}
