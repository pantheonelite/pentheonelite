...
export const api = {
  ...
  getAgents: async (): Promise<Agent[]> => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/v1/hedge-fund/agents`);
      if (!response.ok) {
  ...
  saveJsonFile: async (filename: string, data: any): Promise<void> => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/v1/storage/save-json`, {
        method: 'POST',
        headers: {
  ...
  runHedgeFund: (
    ...
    fetch(`${API_BASE_URL}/api/v1/hedge-fund/run`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
