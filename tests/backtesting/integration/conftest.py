...
PRICES_ROOT = (
    Path(__file__).resolve().parents[3] / "tests" / "fixtures" / "api" / "prices"
)
FM_ROOT = (
...
def _load_price_df_from_fixture(ticker: str, start: str, end: str) -> pd.DataFrame:
    ...
    assert fixture_path is not None, (
        f"Missing price fixture for {ticker} covering {start}..{end}"
    )
    with fixture_path.open("r") as f:
        data = json.load(f)
    # Build DataFrame similar to prices_to_df output
    df = pd.DataFrame(list(data["prices"]))
    df["Date"] = pd.to_datetime(df["time"]).dt.tz_convert(
        "UTC"
    )  # align with prices_to_df index name
...
def _load_financial_metrics_from_fixture(
    ticker: str, end: str, limit: int
) -> list[dict]:
    fixture_path = _find_fm_fixture_file(ticker, end)
    assert fixture_path is not None, (
        f"Missing financial metrics fixture for {ticker} covering ..{end}"
    )
    with fixture_path.open("r") as f:
        data = json.load(f)
...
def _load_news_from_fixture(
    ticker: str, start: str | None, end: str, limit: int
) -> list[dict]:
...
def _load_insider_from_fixture(
    ticker: str, start: str | None, end: str, limit: int
) -> list[dict]:
...
def patch_engine_prices(monkeypatch):
    ...
    monkeypatch.setattr(
        "src.backtesting.engine.get_company_news", _fake_get_company_news
    )
