...
def test_long_only_strategy_buys_and_sells():
    ...
    assert positions["AAPL"]["long"] == expected_final_aapl, (
        f"AAPL position mismatch: expected {expected_final_aapl} shares, got {positions['AAPL']['long']}"
    )
    assert positions["MSFT"]["long"] == 30, (
        f"MSFT position mismatch: expected 30 shares, got {positions['MSFT']['long']}"
    )
    assert positions["TSLA"]["long"] == 0, (
        f"TSLA position mismatch: expected 0 shares, got {positions['TSLA']['long']}"
    )

    # Verify the AAPL sale generated realized gains (proves sale happened)
    assert realized_gains["AAPL"]["long"] != 0.0, (
        "AAPL should have realized gains from sale"
    )
    assert realized_gains["MSFT"]["long"] == 0.0, (
        "MSFT should have no realized gains (no sales)"
    )
    ...
    assert actual_return_pct == expected_return_pct, (
        f"Return percentage should {expected_return_pct}"
    )
    ...
    assert final_portfolio_value == expected_total_value, (
        f"Final portfolio value should be {expected_total_value}"
    )
...
def test_long_only_strategy_full_liquidation_cycle():
    ...
    assert positions["TSLA"]["long"] == 0, (
        f"TSLA should be fully sold, got {positions['TSLA']['long']}"
    )

    # Should have no short positions (long-only strategy)
    for ticker in tickers:
        assert positions[ticker]["short"] == 0, (
            f"Expected no short position in {ticker}"
        )
    ...
    assert realized_gains["MSFT"]["long"] != 0.0, (
        "MSFT should have realized gains from sale"
    )
    assert realized_gains["TSLA"]["long"] != 0.0, (
        "TSLA should have realized gains from sale"
    )

    # Cost basis should be reset to zero after complete sales
    assert positions["AAPL"]["long_cost_basis"] == 0.0, (
        "AAPL cost basis should be reset to 0"
    )
    assert positions["MSFT"]["long_cost_basis"] == 0.0, (
        "MSFT cost basis should be reset to 0"
    )
    assert positions["TSLA"]["long_cost_basis"] == 0.0, (
        "TSLA cost basis should be reset to 0"
    )
    ...
    expected_return_pct = (final_portfolio_value / initial_capital - 1.0) * 100.0
    assert actual_return_pct == expected_return_pct, (
        f"Return percentage should be {expected_return_pct}"
    )
...
def test_long_only_strategy_portfolio_rebalancing():
    ...
    decision_sequence = [
        ...
        # Day 2: First rebalance - reduce AAPL, add TSLA
        {
            "AAPL": {
                "action": "sell",
                "quantity": 40,
            },  # Sell 40 of 100 AAPL (60 remaining)
    ...
    assert positions["AAPL"]["long"] == final_aapl_expected, (
        f"AAPL position mismatch: expected {final_aapl_expected} shares, got {positions['AAPL']['long']}"
    )
    assert positions["MSFT"]["long"] == final_msft_expected, (
        f"MSFT position mismatch: expected {final_msft_expected} shares, got {positions['MSFT']['long']}"
    )
    assert positions["TSLA"]["long"] == final_tsla_expected, (
        f"TSLA position mismatch: expected {final_tsla_expected} shares, got {positions['TSLA']['long']}"
    )
    ...
    assert realized_gains["AAPL"]["long"] != 0.0, (
        "AAPL should have realized gains from partial and complete sales"
    )
    assert realized_gains["MSFT"]["long"] == 0.0, (
        "MSFT should have no realized gains (only bought, never sold)"
    )
    assert realized_gains["TSLA"]["long"] == 0.0, (
        "TSLA should have no realized gains (only bought, never sold)"
    )

    # AAPL should have zero cost basis (completely sold)
    assert positions["AAPL"]["long_cost_basis"] == 0.0, (
        "AAPL cost basis should be reset to 0 after complete sale"
    )
    # MSFT and TSLA should have positive cost bases (still holding)
    assert positions["MSFT"]["long_cost_basis"] > 0.0, (
        "MSFT should have positive cost basis (still holding)"
    )
    assert positions["TSLA"]["long_cost_basis"] > 0.0, (
        "TSLA should have positive cost basis (still holding)"
    )
    ...
    assert portfolio_summary["total_position_value"] > 0.0, (
        "Should have position value after rebalancing"
    )
    ...
    expected_min_position_value = (
        initial_capital * 0.15
    )  # At least 15% should be in positions after rebalancing
...
def test_long_only_strategy_multiple_entry_exit_cycles():
    ...
    decision_sequence = [
        {
            "AAPL": {"action": "buy", "quantity": 60},
            "MSFT": {"action": "buy", "quantity": 20},
        },
        {
            "AAPL": {"action": "sell", "quantity": 60},
            "MSFT": {"action": "sell", "quantity": 20},
        },
        {
            "AAPL": {"action": "buy", "quantity": 30},
            "MSFT": {"action": "buy", "quantity": 10},
        },
        {
            "AAPL": {"action": "sell", "quantity": 30},
    ...
    assert positions["AAPL"]["long"] == 0, (
        f"AAPL should be fully sold after cycles, got {positions['AAPL']['long']}"
    )
    assert positions["MSFT"]["long"] == 0, (
        f"MSFT should be fully sold after cycles, got {positions['MSFT']['long']}"
    )
    assert positions["TSLA"]["long"] == 0, (
        f"TSLA should be 0 shares (never traded), got {positions['TSLA']['long']}"
    )
    ...
    assert realized_gains["AAPL"]["long"] != 0.0, (
        "AAPL should have realized gains/losses from multiple cycles"
    )
    assert realized_gains["MSFT"]["long"] != 0.0, (
        "MSFT should have realized gains/losses from multiple cycles"
    )
    assert realized_gains["TSLA"]["long"] == 0.0, (
        "TSLA should have no realized gains (not traded)"
    )

    # Cost basis should be reset to zero for AAPL and MSFT after final full exit
    assert positions["AAPL"]["long_cost_basis"] == 0.0, (
        "AAPL cost basis should reset to 0 after full exit"
    )
    assert positions["MSFT"]["long_cost_basis"] == 0.0, (
        "MSFT cost basis should reset to 0 after full exit"
    )
